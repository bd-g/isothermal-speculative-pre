...
namespace ISPRE
{
    struct ISPREPass : public FunctionPass
    {
        ...
        */
        ...
        // for each expression e of the type x op y, get its operands and search in full function if there is a store to any of them. If yes, then add to kills set
        // if operand is of type load instruction, then get it's operand and search for store with same dest
        // if not of type load instruction but of type mul,sub,add then get it's operands and search for load. Then get load's operand and search for corresponding store with same dest.
        // If found, enter into kills set
            ...
            for (BasicBlock &BB : F)
            {
                ...
                for (auto &instr : BB)
                {
                    ...
                    case Instruction::SRem:
                    {
                        ...
                        {
                            Value *currentOperand = instr.getOperand(idx);
                            Instruction *getOperandInstr = dyn_cast<Instruction>(currentOperand); // crashing if its a constant so check if not nullptr

                            if (getOperandInstr != nullptr && getOperandInstr->getOpcode() == Instruction::Load) // get load for each operand
                            {
                                ...
                                for (BasicBlock &BB1 : F)
                                {
                                    ...
                                    for (auto &instr1 : BB1)
                                    {
                                        if (instr1.getOpcode() == Instruction::Store)
                                        {
                                            ...
                                            if (storeDest == loadOperand) // enter e into kill set of BB1 if store dest is same as load source
                                            {
                                                ...
                                                {
                                                    std::set<Instruction *> instructionToBeAdded = kills[BB1.getName()];
                                                    instructionToBeAdded.insert(&instr);
                                                    kills[BB1.getName()] = instructionToBeAdded;
                                                }
                            ...
                            else if (getOperandInstr != nullptr && (getOperandInstr->getOpcode() == Instruction::Mul || getOperandInstr->getOpcode() == Instruction::Add || getOperandInstr->getOpcode() == Instruction::Sub))
                            {
                                ...
                                for (int idx1 = 0; idx1 < numOfOperands1; idx1++)
                                {
                                    ...
                                    Instruction *getOperandInstr1 = dyn_cast<Instruction>(currentOperand1);
                                    if (getOperandInstr1 != nullptr && getOperandInstr1->getOpcode() == Instruction::Load)
                                    {
                                        ...
                                        for (BasicBlock &BB2 : F)
                                        {
                                            ...
                                            for (auto &instr2 : BB2)
                                            {
                                                if (instr2.getOpcode() == Instruction::Store)
                                                {
                                                    ...
                                                    if (storeDest1 == loadOperand1) // enter e into kill set of BB1 if store dest is same as load source
                                                    {
                                                        if (kills.find(BB2.getName()) == kills.end())
                                                        {
                                                            std::set<Instruction *> instructionToBeAdded1;
                                                            instructionToBeAdded1.insert(&instr);
                                                            kills[BB2.getName()] = instructionToBeAdded1;
                                                        }
                                                        else
                                                        {
                                                            std::set<Instruction *> instructionToBeAdded1 = kills[BB2.getName()];
                                                            instructionToBeAdded1.insert(&instr);
                                                            kills[BB2.getName()] = instructionToBeAdded1;
                                                        }
                                                        break; // go to next BB and check
